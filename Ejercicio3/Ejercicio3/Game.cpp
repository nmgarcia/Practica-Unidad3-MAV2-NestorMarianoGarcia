...
{ 
	...
	pelota->Actualizar(); // Actualiza la posición del avatar
	pelota->Dibujar(*wnd); // Dibuja el avatar en la ventana
...
void Game::InitPhysics()
{
	...
	pelotaBody->SetTransform(b2Vec2(10.0f, 10.0f), 0.0f);
	
	cajaBody = Box2DHelper::CreateRectangularStaticBody(phyWorld, 10.0f, 10.0f);
	cajaBody->SetTransform(b2Vec2(50.0f, 10.0f), 0.0f);	
	
	Box2DHelper::CreateDistanceJoint(phyWorld, pelotaBody, pelotaBody->GetWorldCenter(), cajaBody,cajaBody->GetWorldCenter(), 1.0f, 0.5f, 0.5f);
	...
	pelota = new Avatar(pelotaBody, new sf::Sprite(texturaPelota));	
	caja = new Avatar(cajaBody, new sf::Sprite(texturaCaja));
